...
## Los ficheros van con permiso temporal, no con URL pública

Los tres buckets (`content-videos`, `content-covers`, `boombastic`) están **cerrados**
desde el 2026-08-13. Antes eran públicos: cualquiera con la dirección se descargaba
un contrato sin sesión — comprobado ese día con un adjunto real (HTTP 200, 620 KB).

**Lo que hay en las columnas NO es una URL que funcione: es un identificador.**
`projects.cover_url`, `content_agenda.cover_url`, `task_attachments.url`… siguen
guardando una cadena con forma de URL pública, y `rutaDeStorage()` le saca bucket y
ruta. Las rutas de lectura la cambian por una URL firmada (1 h) justo antes de
responder, con `firmarCampos()` de `src/lib/storageFirmado.ts`.

Se hizo así para no migrar datos ni tocar los **17 sitios de la UI** que pintan
ficheros: siguen escribiendo `src={item.cover_url}` sin enterarse de nada.

**Si añades una ruta que devuelva un campo con ficheros, llama a `firmarCampos()`
antes del `NextResponse`.** Si se te olvida, esa pantalla sale con las imágenes
rotas. Es un fallo visible, no silencioso, y se prefirió eso a dejar el bucket abierto.

Dos casos que no encajan en ese molde y tienen su propia solución:

- **Subidas que pintan al momento** (`upload-cover`, `analyze-pdf`, el POST de
  adjuntos): guardan el identificador y **devuelven la firmada**, porque la UI la
  pinta antes de recargar nada.
- **Enlaces dentro de texto libre.** `MemoriaSection` guarda el documento dentro
  del cuerpo de la nota, y ahí no se puede firmar al leer. Por eso existe
  **`/api/archivo?u=…`**: comprueba la sesión y redirige a una firma fresca. El
  enlace guardado no caduca y no le sirve a quien no tenga cuenta. Valida el destino
  con `isOwnStorageUrl` — sin eso sería un redirector abierto, que es lo que hace
  creíble un phishing.

Los `createBucket(...)` van con **`public: false`**. Hoy no hacen nada porque los
buckets existen, pero si alguien los borrara, recrearlos abiertos devolvería los
contratos a la intemperie.

Y nadie hace ya `fetch()` de una URL del Storage desde el servidor: `analyze-pdf` y
`documents` bajan por la API con el service role. Además de que con el bucket
cerrado ese fetch daría 400, quitó del medio una petición saliente del servidor a
una URL que llegaba en el body — con `documents` devolviendo el resumen que Claude
hace de lo descargado, era una primitiva de lectura con canal de salida.

## Trabajo pendiente que NO debe hacerse a la ligera

**Los dos bloques que vivían aquí están CERRADOS** (2026-08-13). Se cerraron después
de hacer inventario de solo lectura contra producción, y resultó que los planes
rechazados (15 y 17 roturas) describían un problema que ya no existía: el embed
`co_assignee` **ya funcionaba**, y de las columnas que supuestamente faltaban solo
faltaban dos. La lección: **mide contra la base antes de planificar, no después.**

Lo que queda de aquellos planes vive en `docs/sql-rechazado/`. **No los ejecutes**:
describen un esquema que ya no se parece al real.
