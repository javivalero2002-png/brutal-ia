...
import { PLATAFORMA_COLOR, useIsMobile, BLU, RED, GRN, BORDER, SURF2, LucideIcon, dlDate, dlLabel, estadoDeadline, Donut, Gauge, AreaChart, localDayKey, parseImporte, colorEstadoCliente } from '@/components/shared'
    ...
    <div className={isMobile ? 'h-full overflow-y-auto p-4' : 'p-6 max-w-[1100px] mx-auto h-full overflow-y-auto'}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3 flex-shrink-0">
        ...
        <button onClick={()=>{
          ...
          //
          // El informe llevaba tareas, proyectos y diario —gestión interna— y
          // omitía justo lo que el estudio ENTREGA. Para un estudio de contenido,
          // llevar a una reunión un informe sin las piezas publicadas es enseñar
          // el andamio y no el edificio. El dato ya estaba en pantalla.
                ...
                <div key={c.id} className="flex items-center gap-3 py-2.5" style={{borderBottom:i<rows.length-1?'1px solid rgba(255,255,255,0.04)':'none'}}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="font-syne text-[7px] font-black px-1.5 py-0.5 rounded-full flex-shrink-0" style={{background:`${colorEstadoCliente(c.status)}14`,color:c.status==='Archivado'?'rgba(255,255,255,0.25)':colorEstadoCliente(c.status)}}>{c.status.toUpperCase()}</span>
                    </div>
                    {avgProg !== null && (
